...
export default function ExploreTopics() {
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        // Check cache first
        const cached = sessionStorage.getItem('explore-topics');
        const cacheTime = sessionStorage.getItem('explore-topics-time');
        const now = Date.now();
        ...
        // Cache the results
        sessionStorage.setItem('explore-topics', JSON.stringify(data));
        sessionStorage.setItem('explore-topics-time', now.toString());
        setTopics(data);
      } catch (error) {
  ...
  return (
    <section
      id="topics"
      className="py-12 sm:mt-20 overflow-y-auto max-h-[800px] min-h-[700px] md:px-6 mb-20 relative"
    >
      <TextStretching
        text="Explore Topics!"
        flex={true}
        ...
        minFontSize={20}
      />
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 max-w-7xl mx-auto relative z-10 py-10 gap-6">
        {loading ? (
...
const TopicCard = ({ title, description, icon, onClick, index }) => {
  return (
    <div
      onClick={onClick}
      className={cn(
        "cursor-pointer flex flex-col py-10 relative group/feature rounded-lg shadow-md transition-transform duration-300 hover:scale-105 hover:shadow-xl",
...
const TopicCardSkeleton = () => {
  return (
    <div className="flex flex-col lg:border-r py-10 relative group/feature rounded-lg shadow-md bg-white dark:bg-neutral-900 animate-pulse">
